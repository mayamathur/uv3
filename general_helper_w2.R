...
make_url_key = function( dat,
  ...
  #ordered.urls = as.character( dat[1, cols] )
  ordered.urls = vapply( dat[1, cols], as.character, "asdf")
...
reorder_random_thing = function( randomized.urls, 
  ...
  # ~~~ NEW: BETTER VERSION THAT ALLOWS FOR L&M THAT USES ONLY A SUBSET OF LOOPS
  # has length equal to number of key.urls (all possible stimuli) and NA for any possible
  #  stimuli that weren't shown to subject
  inds = match(key.urls, randomized.urls)
...
split_on_face = function( data,
  ...
  if ( any( is.na( cant.be.na ) ) ) {
    warning( "w2_uID ", data$w2_uID[id], " should be excluded. Idiosyncratic timing issues caused missing times or outcome variable data.")
    # look for global variable that is tracking subjects to exclude
    if ( exists("exclusions") ) exclusions <<- rbind( exclusions, data.frame( w2_uID = data$w2_uID[id],
                                                                            reason = "Idiosyncratic timing issues caused missing times or outcome variable data.") )
    return(NA)
    }
  
  if ( max(onReadyTime) > max(t) ) {
    warning( "w2_uID ", data$w2_uID[id], " should be excluded. Continuous timing stopped prematurely.")
    # look for global variable that is tracking subjects to exclude
    if ( exists("exclusions") ) exclusions <<- rbind( exclusions, data.frame( w2_uID = data$w2_uID[id],
                                                                              reason = "Idiosyncratic timing issues caused missing times or outcome variable data.") )
    return(NA)
  ...
  list = lapply( 1:n.faces,
                 FUN = function(i) {
                   ...
                   if ( any( buttonClickTime < 0 ) | any( onReadyTime < 0 ) ){ 
                     warning( "w2_uID ", data$w2_uID[id], " should be excluded. One or more button click or on-ready times were negative.")
                     # look for global variable that is tracking subjects to exclude
                     if ( exists("exclusions") ) exclusions <<- rbind( exclusions, data.frame( w2_uID = data$w2_uID[id],
                                                                                               reason = "Idiosyncratic timing issues caused missing times or outcome variable data.") )
                     return(NA)
                   ...
                   if ( any( diff(buttonClickTime) < 0 ) | any( diff(onReadyTime) < 0 ) ){ 
                     warning( "w2_uID ", data$w2_uID[id], " should be excluded. Non-monotonic button click or on-ready times.")
                     # look for global variable that is tracking subjects to exclude
                     if ( exists("exclusions") ) exclusions <<- rbind( exclusions, data.frame( w2_uID = data$w2_uID[id],
                                                                                               reason = "Idiosyncratic timing issues caused missing times or outcome variable data.") )
                     return(NA)
                   ...
                   if ( !is.na(buttonClickTime[i]) ) {
                     ...
                     if ( length(start.ind) == 0 | length(end.ind) == 0 ) {
                       ...
                       warning( "w2_uID ", data$w2_uID[id], " should be excluded. No valid times within a certain button click/on ready interval.")
                       # look for global variable that is tracking subjects to exclude
                       if ( exists("exclusions") ) exclusions <<- rbind( exclusions, data.frame( w2_uID = data$w2_uID[id],
                                                                                                 reason = "Idiosyncratic timing issues caused missing times or outcome variable data.") )
                       return(NA)
  ...
  if ( reorder == TRUE & !is.na(key.urls[1]) ) {
    ...
    if ( any( randomized.urls == "" ) ) {
      warning( "w2_uID ", data$w2_uID[id], " should be excluded. Idiosyncratic issues caused a failure to record one or more stimulus URLs.")
      # look for global variable that is tracking subjects to exclude
      if ( exists("exclusions") ) exclusions <<- rbind( exclusions, data.frame( w2_uID = data$w2_uID[id],
                                                                                reason = "Idiosyncratic timing issues caused missing times or outcome variable data.") )
      return(NA)
...
get_subject_lists = function( data,
  ...
  if ( any( mins < 5 ) ) {
    ...
    for ( j in bad.rows) {
      warning( "w2_uID(s) ", data$w2_uID[j], " should be excluded for having implausibly few (<5) coordinate or time entries for some stimuli.")
      if ( exists("exclusions") ) exclusions <<- rbind( exclusions, data.frame( w2_uID = data$w2_uID[j],
      ...
      for ( j in 1:length(list[[i]]) ) {
        ...
        if ( var.name == "xPos" ) {
         ...
         # distance between first and last entry of subject i, face j vector
          ...
          if( length(x.dist) == 0) {
            cat("\nHitting browser because length(x.dist) == 0")
            browser()
          }
          
          if( is.na(x.dist) ) {
            
            # cat("\nHitting browser because is.na(x.dist) == TRUE")
            # browser()
            
            warning( "w2_uID(s) ", data$w2_uID[i], " had no x-coordinates recorded for at least some faces")
            
            # temporarily set their x-distance so that the function can proceed
            x.dist = 9999
            
            exclusions <<- rbind( exclusions, data.frame( w2_uID = data$w2_uID[i],
                                                          reason = "Coordinates failed to record." ) )
          }
          
          if ( x.dist < min | x.dist > max ) {
          
            warning( "w2_uID(s) ", data$w2_uID[i], " had nonstandard pixel dimensions (x dist = ", x.dist, ")")
...
list_to_df = function( list,
  ...
  # ~~ NEW
  # if only 1 subject, sometimes res is transposed?
  # it's supposed to have 1 column per face
  if ( ncol(res) == 1 & nrow(res) == n.stim ){
